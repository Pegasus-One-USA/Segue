...
> **Scope note:** this section covers destination-independence requirements **#4** ("mapping stored as JSON, no `MappingFields` table") and **#6** ("the same mapping JSON authored in the UI is used at workflow execution when writing to the destination"). It is **orthogonal to customer-owned schema** — it changes how FHIRBridge stores the mapping in **its own** database, not anything about the customer's destination. It is included here because the new `IsUpsertKey` (section 2a) and "Source EHR/System Type" (section 2c) fields ride inside this JSON, so landing them together avoids a second migration.
...
Because FHIRBridge injects **no** identity/rowid column and doesn't own the schema, a child FK must reference a **mapped business key** the parent row actually carries — the parent's `IsUpsertKey` column (e.g. `Patient.Id` sourced from the FHIR resource id). This avoids any need to read back a DB-generated identity FHIRBridge can't see. Validation (§3.C) gains: for every `Child` table, confirm `referencesParentColumn` equals the parent's `IsUpsertKey` `TargetField`, and `foreignKeyColumn` exists on the child table per the live schema (§2b).
