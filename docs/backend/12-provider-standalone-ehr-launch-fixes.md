...
### CSV destination: per-resource ZIP export, mapped-columns-only output

Added after the initial round of fixes above — the CSV Destination Wizard previously produced one CSV mixing every selected resource's differently-shaped fields together, and always included internal lineage columns the user never mapped.

- `src/FHIRBridge.Infrastructure/Destinations/MappedCsvDestinationWriter.cs`
  - Groups mapped records by FHIR resource type. **Single resource selected** → delivered directly as one plain CSV, named `{ResourceType}_{yyyyMMdd_HHmmss}.csv`. **Multiple resources selected** → bundled into a single ZIP, named `{WorkflowName}_{yyyyMMdd_HHmmss}.zip`, containing one `{ResourceType}_{yyyyMMdd_HHmmss}.csv` entry per resource. Delivery mode dispatch (Download/Email/SFTP/Download-link) unchanged.
- `src/FHIRBridge.Infrastructure/Destinations/MappedDestinationSerialization.cs`
  - Added `ToMappedOnlyCsv` — same CSV logic as the existing `ToCsv`, but omits the five lineage columns (`PipelineRunId`, `ResourceType`, `DestinationObject`, `SourceResourceId`, `WrittenOnUtc`) so the exported CSV contains exactly the mapped business fields, nothing else. Added as a new method rather than changing `ToCsv` itself, since `ToCsv`/`GetColumns` are shared by the Excel/Avro/Parquet/PDF/Databricks/Tableau writers — this change is scoped to CSV only.
- `src/Runtime/FHIRBridge.Runtime.Infrastructure/Workflows/Executors/DestinationNodeExecutors.cs`
  - Added a new optional `IWorkflowDefinitionStore` constructor dependency on `DestinationNodeExecutor` and all 21 concrete destination-node subclasses. `PipelineWriteContext.RouteName` (used for the ZIP filename above, and for every destination type's `{{RouteName}}` email-template placeholder) previously came from `node.NodeType` — a literal string like `"CsvDestinationNode"` — for every Runtime-Plane-triggered destination write. Now resolves the actual `WorkflowDefinition.Name` via `node.WorkflowDefinitionId`, falling back to the old `node.NodeType` value if the store is unavailable.
- **Related bug found and fixed while verifying delivery paths**: `src/FHIRBridge.Infrastructure/Destinations/SftpUploader.cs` took a `string content` and did `Encoding.UTF8.GetBytes`/`GetString` round-trips in its two callers (`MappedSftpDestinationWriter.cs`, `Delivery/SftpDeliveryStrategy.cs`) — harmless for the old plain-text CSV/NDJSON payloads, but would have silently corrupted the new binary ZIP output if SFTP delivery were selected for a multi-resource CSV export. Changed `SftpUploader.UploadAsync` to take `byte[]` directly; updated both callers accordingly.
- `tests/FHIRBridge.UnitTests/Destinations/MappedCsvDestinationWriterTests.cs` — rewritten: single-resource → plain CSV assertions, multi-resource → ZIP named after the workflow (with per-entry naming/content assertions), and a dedicated test asserting the CSV header is exactly the mapped columns with none of the five lineage columns present.
