...
| **§164.312(a)(2)(i) Unique User Identification** | Every principal authenticates as a unique user identity — local accounts (PBKDF2-SHA256, 350,000 iterations) or federated Entra SSO identity. User identity is carried on the JWT and stamped into the hash-chained `UserActivityAuditLog`, giving per-user attribution of actions. | Implemented |
...
| **§164.312(b) Audit Controls** | **Hash-chained, append-only `UserActivityAuditLog`** with `GuardAppendOnlyLogs` preventing update/delete of audit rows (tamper-evidence via chained hashes). **Serilog** structured logging with a **`PhiMaskingEnricher`** that masks PHI before logs are written. Data-access events are recorded PHI-free. | Implemented |
| **§164.312(c)(1) Integrity** | Audit log integrity via hash chaining + append-only guard (any tampering breaks the chain and is detectable). Application-layer input validation and typed FHIR/HL7 processing preserve message integrity. TLS protects data in transit from modification (see (e)(1)). | Implemented |
...
| **CC7.1 — Detection of configuration changes / vulnerabilities (monitoring)** | Startup TDE health check surfaces at-rest encryption misconfiguration; structured Serilog logging enables monitoring pipelines; append-only audit trail supports detection of anomalous access. (Vulnerability scanning / IDS tooling is organizational.) | Implemented + Organizational |
| **CC7.2 — Monitoring of system components for anomalies (security events)** | Hash-chained `UserActivityAuditLog` + `GuardAppendOnlyLogs` provide tamper-evident security-event records; per-user attribution; rate-limit rejections and lockout events are observable via logs. (Alerting/SIEM integration is organizational.) | Implemented + Organizational |
