...
# storage.tf) and registered in Cloud Map instead of the ALB. segue-app sits behind the ALB;
# worker has neither.
...
resource "aws_ecs_service" "seq" {
  ...
  # Both at once: Cloud Map so segue_app/worker can reach Seq internally by hostname to ship
...
resource "aws_ecs_service" "segue_app" {
  name            = "${var.name_prefix}-app"
  cluster         = aws_ecs_cluster.main.id
  task_definition = aws_ecs_task_definition.segue_app.arn
  desired_count   = 1
  launch_type     = "FARGATE"
  ...
  # local.seguedb_connection_string references aws_db_instance.postgresql directly, so
  ...
  depends_on = [aws_ecs_service.postgres, aws_ecs_service.redis, aws_ecs_service.seq, aws_lb_listener.segue_app]
...
resource "aws_ecs_service" "worker" {
  ...
  # retry.
  depends_on = [aws_ecs_service.postgres, aws_ecs_service.redis, aws_ecs_service.seq, aws_ecs_service.segue_app]
