...
- **Custom domain + free managed certificate is supported** via the `fhirbridgeAppCustomDomain`/
  `demoAppCustomDomain` parameters (blank by default, same two-phase deploy as the Terraform Azure
  environment — see `Documents/Containerization-Setup-Steps.html`), and `createUiDefinition.json`
  has a dedicated "Custom Domains" wizard step for it. This resource type/apiVersion combination
  hasn't been exercised against a live subscription yet in this repo — validate it (`az deployment
  group validate`, or a real first deploy) before relying on it for a client. Leave both parameters
  blank to keep plain HTTPS via Container Apps' built-in ingress with no custom domain at all.
