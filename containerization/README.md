...
| 3 | `sqlserver` | `mcr.microsoft.com/mssql/server:2022-latest`, `MSSQL_PID=Express` — hosts both the `Segue` and `HealthAppDb` databases | No |
...
- **Recommended (Azure): import an already-published version, no local Docker needed for this
  ...
  `demo-app`/`fhirbridge-worker`), then set `image_tag = "v1.2.0"` in `terraform.tfvars`.
...
# repeat for demo-app and fhirbridge-worker
```
...
- **TLS**: Azure Container Apps' public ingress (`segue-app`/`demo-app`) gets automatic,
...
- **Secrets**: AWS reads `sql_sa_password`/`jwt_signing_key`/`redis_password` out of AWS Secrets
  ...
  `environment`). Azure seeds the same 3 values into a Key Vault (Access Policy authorization, not
  RBAC — RBAC role assignments need Owner/User Access Administrator, a narrower permission than
  many Contributor-scoped accounts have) on first apply, and every Container App reads the Key
  Vault secret's value from then on — rotate via
