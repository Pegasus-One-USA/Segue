  ...
  implements OnInit, HasUnsavedChanges, SourceConfigFormComponent
{
  ...
  protected save(): void {
    ...
    // WizardService.scopeString() (what save() actually sends — see its own remarks) needs this to inject
    // system/Group.read; this form's own scopeString() computed is preview-only and never reaches the request.
    this.wiz.groupBulkExport.set(!!v.groupBulkExport);
