...
export type AthenahealthRetrievalMethod = 'subscription' | 'webhook' | 'search-rest' | 'bulk-export';
export type AthenahealthRunMode = 'incremental' | 'full' | 'manual';
export type AthenahealthExportScope = 'system' | 'group' | 'patient';

const RETRIEVAL_METHOD_OPTIONS: readonly { value: AthenahealthRetrievalMethod; label: string; description: string }[] = [
...
/**
 * Source-config form for Athenahealth — a conformant FHIR R4 server
 * (SourceSystemType.Athenahealth). Started as a copy of GenericFhirSourceFormComponent (see that file's own
 * doc comment for the full rationale on field choices/parity with Epic's form) since Athenahealth is
 ...
 * Athenahealth-specific auth or resource constraints are added, without touching the other vendors.
 */
@Component({
  selector: 'app-athenahealth-source-form',
  standalone: true,
  imports: [ReactiveFormsModule],
  templateUrl: './athenahealth-source-form.component.html',
  styleUrls: [
    '../../epic-source-wizard/epic-audience-form/epic-audience-form.component.scss',
    './athenahealth-source-form.component.scss',
  ],
})
export class AthenahealthSourceFormComponent {
  ...
  readonly form = this.fb.group({
    name: ['Athenahealth', [Validators.required]],
    baseUrl: ['', [Validators.required]],
    retrievalMethod: ['search-rest' as AthenahealthRetrievalMethod],
    ...
    // ── Search REST ──────────────────────────────────────────────────────────
    searchCriteria: [''],
    runMode: ['manual' as AthenahealthRunMode],
    schedulePollFrequency: [''],
    // ── Bulk $export ─────────────────────────────────────────────────────────
    exportScope: ['system' as AthenahealthExportScope],
    groupId: [''],
    patientIds: [''],
  ...
  private _populate(fields: Record<string, string>): void {
    this.form.patchValue({
      ...
      retrievalMethod: (fields['Retrieval method key'] as AthenahealthRetrievalMethod) || 'search-rest',
      ...
      runMode: (fields['Run mode'] as AthenahealthRunMode) || 'manual',
      schedulePollFrequency: fields['Schedule / poll frequency'] || '',
      exportScope: (fields['Export scope'] as AthenahealthExportScope) || 'system',
  ...
  getFields(): Record<string, string> | null {
    ...
    const fields: Record<string, string> = {
      __name: v.name || 'Athenahealth',
      Connector: 'Athenahealth',
      'App context': 'Provider (standalone)',
