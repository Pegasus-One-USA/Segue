...
@Component({
  ...
  styleUrl: './athenahealth-source-form.component.scss',
})
export class AthenahealthSourceFormComponent implements OnInit, HasUnsavedChanges, SourceConfigFormComponent {
  /** This component is permanently configured for the Athenahealth vendor. It used to be a thin wrapper around a
   *  shared EhrVendorSourceFormComponent engine driven by a `vendor` input; that engine has been fully inlined
   *  here so Athenahealth's configuration, validation, and API handling are completely independent of every
   *  other EHR vendor's form — editing this file can never affect Epic/Cerner/Allscripts/Healow/Meditech/Sample.
   *  Every `this.vendor()` call below is unchanged from the original shared implementation. */
  protected readonly vendor = (): EhrVendor => 'Athenahealth';
  ...
  protected hasExistingChanged(): boolean {
    ...
    const pick = (v: Record<string, unknown>) =>
      Object.fromEntries(Object.entries(v).filter(([key]) =>
        AthenahealthSourceFormComponent.CONNECTION_IDENTITY_FIELDS.has(key)));
  ...
  private buildFieldsToSave(
    v: ReturnType<AthenahealthSourceFormComponent['form']['getRawValue']>,
    aud: EpicAudience,
