...
/**
 * Thin, independently-editable wrapper around the shared EhrVendorSourceFormComponent engine, fixed to the Athenahealth
 * vendor. Forwards every input/output the engine exposes unchanged — the only thing this file owns is "which
 * vendor" (see the `[vendor]="'Athenahealth'"` binding in the template). Registered in source-form.registry.ts under the
 * `athena` key from sources.data.ts.
 */
@Component({
...
export class AthenahealthSourceFormComponent implements SourceConfigFormComponent {
