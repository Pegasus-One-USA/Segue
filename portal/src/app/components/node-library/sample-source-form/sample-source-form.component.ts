...
export type SampleRetrievalMethod = 'subscription' | 'webhook' | 'search-rest' | 'bulk-export';
export type SampleRunMode = 'incremental' | 'full' | 'manual';
export type SampleExportScope = 'system' | 'group' | 'patient';

const RETRIEVAL_METHOD_OPTIONS: readonly { value: SampleRetrievalMethod; label: string; description: string }[] = [
...
/**
 * Source-config form for Sample (sandbox) — a conformant FHIR R4 server
 * (SourceSystemType.Sample). Started as a copy of GenericFhirSourceFormComponent (see that file's own
 * doc comment for the full rationale on field choices/parity with Epic's form) since Sample (sandbox) is
 ...
 * Sample (sandbox)-specific auth or resource constraints are added, without touching the other vendors.
 */
@Component({
  selector: 'app-sample-source-form',
  standalone: true,
  imports: [ReactiveFormsModule],
  templateUrl: './sample-source-form.component.html',
  styleUrls: [
    '../../epic-source-wizard/epic-audience-form/epic-audience-form.component.scss',
    './sample-source-form.component.scss',
  ],
})
export class SampleSourceFormComponent {
  ...
  readonly form = this.fb.group({
    name: ['Sample (sandbox)', [Validators.required]],
    baseUrl: ['', [Validators.required]],
    retrievalMethod: ['search-rest' as SampleRetrievalMethod],
    ...
    // ── Search REST ──────────────────────────────────────────────────────────
    searchCriteria: [''],
    runMode: ['manual' as SampleRunMode],
    schedulePollFrequency: [''],
    // ── Bulk $export ─────────────────────────────────────────────────────────
    exportScope: ['system' as SampleExportScope],
    groupId: [''],
    patientIds: [''],
  ...
  private _populate(fields: Record<string, string>): void {
    this.form.patchValue({
      name: fields['__name'] || 'Sample (sandbox)',
      baseUrl: fields['FHIR base URL'] || '',
      retrievalMethod: (fields['Retrieval method key'] as SampleRetrievalMethod) || 'search-rest',
      ...
      runMode: (fields['Run mode'] as SampleRunMode) || 'manual',
      schedulePollFrequency: fields['Schedule / poll frequency'] || '',
      exportScope: (fields['Export scope'] as SampleExportScope) || 'system',
  ...
  getFields(): Record<string, string> | null {
    ...
    const fields: Record<string, string> = {
      __name: v.name || 'Sample (sandbox)',
      Connector: 'Sample (sandbox)',
