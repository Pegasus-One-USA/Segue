...
export class NodeLibraryDialogComponent {
  ...
  readonly destWizardType   = signal<'sql' | 'csv' | 'mysql' | 'mongo' | 'postgres' | 'fhir' | 'blob' | null>(null);
  ...
  readonly pendingDestSwitch      = signal<'sql' | 'csv' | 'mysql' | 'mongo' | 'postgres' | 'fhir' | 'blob' | null>(null);
  ...
  private readonly allCategories = computed<LibraryCategory[]>(() => {
    ...
    TRANSFORMS.forEach(t => {
      ...
      if (m === 'transform') {
        ...
        if ((t.id === 'dest-sqlserver' || t.id === 'dest-csv' || t.id === 'dest-mysql' || t.id === 'dest-mongo' || t.id === 'dest-postgres' || t.id === 'dest-fhir' || t.id === 'dest-blob') && this.destTypeLocked()) {
          status = 'disabled';
  ...
  selectItem(item: LibraryItem): void {
    ...
    if (item.id === 'dest-sqlserver' || item.id === 'dest-csv' || item.id === 'dest-mysql' || item.id === 'dest-mongo' || item.id === 'dest-postgres' || item.id === 'dest-fhir' || item.id === 'dest-blob') {
      const type: 'sql' | 'csv' | 'mysql' | 'mongo' | 'postgres' | 'fhir' | 'blob' =
        item.id === 'dest-sqlserver' ? 'sql' : item.id === 'dest-mysql' ? 'mysql' : item.id === 'dest-postgres' ? 'postgres' : item.id === 'dest-mongo' ? 'mongo' : item.id === 'dest-fhir' ? 'fhir' : item.id === 'dest-blob' ? 'blob' : 'csv';
  ...
  destTypeLabel(type: 'sql' | 'csv' | 'mysql' | 'mongo' | 'postgres' | 'fhir' | 'blob' | null): string {
    return type === 'sql' ? 'SQL Server' : type === 'mysql' ? 'MySQL' : type === 'postgres' ? 'PostgreSQL' : type === 'mongo' ? 'MongoDB' : type === 'fhir' ? 'FHIR Repository (Aidbox)' : type === 'blob' ? 'Azure Blob Storage' : 'CSV';
  }
  ...
  // ── destination wizard ────────────────────────────────────────────────────
  private _openDestWizard(type: 'sql' | 'csv' | 'mysql' | 'mongo' | 'postgres' | 'fhir' | 'blob'): void {
    const pm = this.pickerModel();
    if (!pm) return;
  ...
  private _openDestWizardEdit(node: CanvasNode): void {
    const tId = (node as TransformNode).transformId;
    const type: 'sql' | 'csv' | 'mysql' | 'mongo' | 'postgres' | 'fhir' | 'blob' =
      tId === 'dest-sqlserver' ? 'sql' : tId === 'dest-mysql' ? 'mysql' : tId === 'dest-postgres' ? 'postgres' : tId === 'dest-mongo' ? 'mongo' : tId === 'dest-fhir' ? 'fhir' : tId === 'dest-blob' ? 'blob' : 'csv';
