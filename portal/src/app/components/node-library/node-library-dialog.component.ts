...
const TRANSFORM_META: Record<string, { abbr: string; color: string }> = {
  ...
  'dest-ahds':        { abbr: 'AHD', color: '#0078D4' },
...
export class NodeLibraryDialogComponent {
  ...
  readonly destWizardType   = signal<'sql' | 'csv' | 'mysql' | 'mongo' | 'postgres' | 'ahds' | null>(null);
  ...
  readonly pendingDestSwitch      = signal<'sql' | 'csv' | 'mysql' | 'mongo' | 'postgres' | 'ahds' | null>(null);
    ...
    effect(() => {
      ...
      if (this.open() && id) {
        ...
        if (node?.kind === 'transform') {
          ...
          if (tId === 'dest-sqlserver' || tId === 'dest-csv' || tId === 'dest-mysql' || tId === 'dest-mongo' || tId === 'dest-postgres' || tId === 'dest-ahds') {
  ...
  private readonly allCategories = computed<LibraryCategory[]>(() => {
    ...
    TRANSFORMS.forEach(t => {
      ...
      if (m === 'transform') {
        ...
        if ((t.id === 'dest-sqlserver' || t.id === 'dest-csv' || t.id === 'dest-mysql' || t.id === 'dest-mongo' || t.id === 'dest-postgres' || t.id === 'dest-ahds') && this.destTypeLocked()) {
          status = 'disabled';
  ...
  selectItem(item: LibraryItem): void {
    ...
    if (item.id === 'dest-sqlserver' || item.id === 'dest-csv' || item.id === 'dest-mysql' || item.id === 'dest-mongo' || item.id === 'dest-postgres' || item.id === 'dest-ahds') {
      const type: 'sql' | 'csv' | 'mysql' | 'mongo' | 'postgres' | 'ahds' =
        item.id === 'dest-sqlserver' ? 'sql' : item.id === 'dest-mysql' ? 'mysql' : item.id === 'dest-postgres' ? 'postgres' : item.id === 'dest-mongo' ? 'mongo' : item.id === 'dest-ahds' ? 'ahds' : 'csv';
  ...
  destTypeLabel(type: 'sql' | 'csv' | 'mysql' | 'mongo' | 'postgres' | 'ahds' | null): string {
    return type === 'sql' ? 'SQL Server' : type === 'mysql' ? 'MySQL' : type === 'postgres' ? 'PostgreSQL' : type === 'mongo' ? 'MongoDB' : type === 'ahds' ? 'Azure Health Data Services' : 'CSV';
  }
  ...
  // ── destination wizard ────────────────────────────────────────────────────
  private _openDestWizard(type: 'sql' | 'csv' | 'mysql' | 'mongo' | 'postgres' | 'ahds'): void {
    const pm = this.pickerModel();
    if (!pm) return;
  ...
  private _openDestWizardEdit(node: CanvasNode): void {
    const tId = (node as TransformNode).transformId;
    const type: 'sql' | 'csv' | 'mysql' | 'mongo' | 'postgres' | 'ahds' =
      tId === 'dest-sqlserver' ? 'sql' : tId === 'dest-mysql' ? 'mysql' : tId === 'dest-postgres' ? 'postgres' : tId === 'dest-mongo' ? 'mongo' : tId === 'dest-ahds' ? 'ahds' : 'csv';
