...
export type MeditechRetrievalMethod = 'subscription' | 'webhook' | 'search-rest' | 'bulk-export';
export type MeditechRunMode = 'incremental' | 'full' | 'manual';
export type MeditechExportScope = 'system' | 'group' | 'patient';

const RETRIEVAL_METHOD_OPTIONS: readonly { value: MeditechRetrievalMethod; label: string; description: string }[] = [
...
/**
 * Source-config form for Meditech Greenfield — a conformant FHIR R4 server
 * (SourceSystemType.MeditechGreenfield). Started as a copy of GenericFhirSourceFormComponent (see that file's own
 * doc comment for the full rationale on field choices/parity with Epic's form) since Meditech Greenfield is
 ...
 * Meditech Greenfield-specific auth or resource constraints are added, without touching the other vendors.
 */
@Component({
  ...
  styleUrls: [
    ...
    './meditech-source-form.component.scss',
  ],
})
export class MeditechSourceFormComponent {
  ...
  readonly form = this.fb.group({
    name: ['Meditech Greenfield', [Validators.required]],
    baseUrl: ['', [Validators.required]],
    retrievalMethod: ['search-rest' as MeditechRetrievalMethod],
    ...
    // ── Search REST ──────────────────────────────────────────────────────────
    searchCriteria: [''],
    runMode: ['manual' as MeditechRunMode],
    schedulePollFrequency: [''],
    // ── Bulk $export ─────────────────────────────────────────────────────────
    exportScope: ['system' as MeditechExportScope],
    groupId: [''],
    patientIds: [''],
  ...
  private _populate(fields: Record<string, string>): void {
    this.form.patchValue({
      name: fields['__name'] || 'Meditech Greenfield',
      baseUrl: fields['FHIR base URL'] || '',
      retrievalMethod: (fields['Retrieval method key'] as MeditechRetrievalMethod) || 'search-rest',
      ...
      runMode: (fields['Run mode'] as MeditechRunMode) || 'manual',
      schedulePollFrequency: fields['Schedule / poll frequency'] || '',
      exportScope: (fields['Export scope'] as MeditechExportScope) || 'system',
  ...
  getFields(): Record<string, string> | null {
    ...
    const fields: Record<string, string> = {
      __name: v.name || 'Meditech Greenfield',
      Connector: 'Meditech Greenfield',
