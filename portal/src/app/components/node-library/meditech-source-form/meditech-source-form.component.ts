...
export class MeditechSourceFormComponent implements OnInit, HasUnsavedChanges, SourceConfigFormComponent {
  /** This component is permanently configured for the MeditechGreenfield vendor. It used to be a thin wrapper
   *  around a shared EhrVendorSourceFormComponent engine driven by a `vendor` input; that engine has been fully
   *  inlined here so Meditech's configuration, validation, and API handling are completely independent of every
   *  other EHR vendor's form — editing this file can never affect Epic/Athenahealth/Cerner/Allscripts/Healow/
   *  Sample. Every `this.vendor()` call below is unchanged from the original shared implementation. */
  protected readonly vendor = (): EhrVendor => 'MeditechGreenfield';
  ...
  protected hasExistingChanged(): boolean {
    ...
    const pick = (v: Record<string, unknown>) =>
      Object.fromEntries(Object.entries(v).filter(([key]) =>
        MeditechSourceFormComponent.CONNECTION_IDENTITY_FIELDS.has(key)));
  ...
  private buildFieldsToSave(
    v: ReturnType<MeditechSourceFormComponent['form']['getRawValue']>,
    aud: EpicAudience,
