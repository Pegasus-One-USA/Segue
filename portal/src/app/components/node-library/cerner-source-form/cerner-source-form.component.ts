...
export type CernerRetrievalMethod = 'subscription' | 'webhook' | 'search-rest' | 'bulk-export';
export type CernerRunMode = 'incremental' | 'full' | 'manual';
export type CernerExportScope = 'system' | 'group' | 'patient';

const RETRIEVAL_METHOD_OPTIONS: readonly { value: CernerRetrievalMethod; label: string; description: string }[] = [
...
/**
 * Source-config form for Cerner (Oracle Health) — a conformant FHIR R4 server
 * (SourceSystemType.Cerner). Started as a copy of GenericFhirSourceFormComponent (see that file's own
 * doc comment for the full rationale on field choices/parity with Epic's form) since Cerner (Oracle Health) is
 ...
 * Cerner (Oracle Health)-specific auth or resource constraints are added, without touching the other vendors.
 */
@Component({
  selector: 'app-cerner-source-form',
  standalone: true,
  imports: [ReactiveFormsModule],
  templateUrl: './cerner-source-form.component.html',
  styleUrls: [
    '../../epic-source-wizard/epic-audience-form/epic-audience-form.component.scss',
    './cerner-source-form.component.scss',
  ],
})
export class CernerSourceFormComponent {
  ...
  readonly form = this.fb.group({
    name: ['Cerner (Oracle Health)', [Validators.required]],
    baseUrl: ['', [Validators.required]],
    retrievalMethod: ['search-rest' as CernerRetrievalMethod],
    ...
    // ── Search REST ──────────────────────────────────────────────────────────
    searchCriteria: [''],
    runMode: ['manual' as CernerRunMode],
    schedulePollFrequency: [''],
    // ── Bulk $export ─────────────────────────────────────────────────────────
    exportScope: ['system' as CernerExportScope],
    groupId: [''],
    patientIds: [''],
  ...
  private _populate(fields: Record<string, string>): void {
    this.form.patchValue({
      name: fields['__name'] || 'Cerner (Oracle Health)',
      baseUrl: fields['FHIR base URL'] || '',
      retrievalMethod: (fields['Retrieval method key'] as CernerRetrievalMethod) || 'search-rest',
      ...
      runMode: (fields['Run mode'] as CernerRunMode) || 'manual',
      schedulePollFrequency: fields['Schedule / poll frequency'] || '',
      exportScope: (fields['Export scope'] as CernerExportScope) || 'system',
  ...
  getFields(): Record<string, string> | null {
    ...
    const fields: Record<string, string> = {
      __name: v.name || 'Cerner (Oracle Health)',
      Connector: 'Cerner (Oracle Health)',
