...
export type AllscriptsRetrievalMethod = 'subscription' | 'webhook' | 'search-rest' | 'bulk-export';
export type AllscriptsRunMode = 'incremental' | 'full' | 'manual';
export type AllscriptsExportScope = 'system' | 'group' | 'patient';

const RETRIEVAL_METHOD_OPTIONS: readonly { value: AllscriptsRetrievalMethod; label: string; description: string }[] = [
...
/**
 * Source-config form for Allscripts (Veradigm) — a conformant FHIR R4 server
 * (SourceSystemType.Allscripts). Started as a copy of GenericFhirSourceFormComponent (see that file's own
 * doc comment for the full rationale on field choices/parity with Epic's form) since Allscripts (Veradigm) is
 ...
 * Allscripts (Veradigm)-specific auth or resource constraints are added, without touching the other vendors.
 */
@Component({
  ...
  styleUrls: [
    ...
    './allscripts-source-form.component.scss',
  ],
})
export class AllscriptsSourceFormComponent {
  ...
  readonly form = this.fb.group({
    name: ['Allscripts (Veradigm)', [Validators.required]],
    baseUrl: ['', [Validators.required]],
    retrievalMethod: ['search-rest' as AllscriptsRetrievalMethod],
    ...
    // ── Search REST ──────────────────────────────────────────────────────────
    searchCriteria: [''],
    runMode: ['manual' as AllscriptsRunMode],
    schedulePollFrequency: [''],
    // ── Bulk $export ─────────────────────────────────────────────────────────
    exportScope: ['system' as AllscriptsExportScope],
    groupId: [''],
    patientIds: [''],
  ...
  private _populate(fields: Record<string, string>): void {
    this.form.patchValue({
      name: fields['__name'] || 'Allscripts (Veradigm)',
      baseUrl: fields['FHIR base URL'] || '',
      retrievalMethod: (fields['Retrieval method key'] as AllscriptsRetrievalMethod) || 'search-rest',
      ...
      runMode: (fields['Run mode'] as AllscriptsRunMode) || 'manual',
      schedulePollFrequency: fields['Schedule / poll frequency'] || '',
      exportScope: (fields['Export scope'] as AllscriptsExportScope) || 'system',
  ...
  getFields(): Record<string, string> | null {
    ...
    const fields: Record<string, string> = {
      __name: v.name || 'Allscripts (Veradigm)',
      Connector: 'Allscripts (Veradigm)',
