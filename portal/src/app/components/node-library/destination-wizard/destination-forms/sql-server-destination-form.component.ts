...
/**
 * Fully independent SQL Server destination form. This used to be a thin wrapper around a shared
 * SqlFamilyDestinationFormComponent engine driven by an `engine` input; that engine has been fully inlined here
 * so SqlServer's configuration, validation, and API handling are completely independent of PostgreSQL/MySQL/
 * AzureSql — editing this file can never affect any of the other three. Every `this.engine()` call below is
 * unchanged from the original shared implementation, just permanently fixed to 'sqlserver'.
 */
@Component({
  ...
  styleUrls: ['../destination-wizard.component.scss', './sql-server-destination-form.component.scss'],
})
export class SqlServerDestinationFormComponent implements SqlFamilyFormApi {
  ...
  private readonly engine = (): 'sqlserver' | 'mysql' | 'postgres' | 'azuresql' => 'sqlserver';
