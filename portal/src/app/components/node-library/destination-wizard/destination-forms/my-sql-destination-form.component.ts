...
/**
 * Fully independent MySQL destination form. This used to be a thin wrapper around a shared
 * SqlFamilyDestinationFormComponent engine driven by an `engine` input; that engine has been fully inlined here
 * so MySQL's configuration, validation, and API handling are completely independent of SqlServer/PostgreSQL/
 * AzureSql — editing this file can never affect any of the other three. Every `this.engine()` call below is
 * unchanged from the original shared implementation, just permanently fixed to 'mysql'.
 */
@Component({
  selector: 'app-my-sql-destination-form',
  standalone: true,
  imports: [ReactiveFormsModule],
  templateUrl: './my-sql-destination-form.component.html',
  styleUrls: ['../destination-wizard.component.scss', './my-sql-destination-form.component.scss'],
})
export class MySqlDestinationFormComponent implements SqlFamilyFormApi {
  ...
  private readonly engine = (): 'sqlserver' | 'mysql' | 'postgres' | 'azuresql' => 'mysql';
