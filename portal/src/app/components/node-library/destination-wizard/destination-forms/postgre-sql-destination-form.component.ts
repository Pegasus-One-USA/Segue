...
/**
 * Fully independent PostgreSQL destination form. This used to be a thin wrapper around a shared
 * SqlFamilyDestinationFormComponent engine driven by an `engine` input; that engine has been fully inlined here
 * so PostgreSQL's configuration, validation, and API handling are completely independent of SqlServer/MySQL/
 * AzureSql — editing this file can never affect any of the other three. Every `this.engine()` call below is
 * unchanged from the original shared implementation, just permanently fixed to 'postgres'.
 */
@Component({
  selector: 'app-postgre-sql-destination-form',
  standalone: true,
  imports: [ReactiveFormsModule],
  templateUrl: './postgre-sql-destination-form.component.html',
  styleUrls: ['../destination-wizard.component.scss', './postgre-sql-destination-form.component.scss'],
})
export class PostgreSqlDestinationFormComponent implements SqlFamilyFormApi {
  ...
  private readonly engine = (): 'sqlserver' | 'mysql' | 'postgres' | 'azuresql' => 'postgres';
