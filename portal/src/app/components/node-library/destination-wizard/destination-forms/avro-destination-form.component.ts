...
/** Minimal stand-in form for the Avro destination type — see SimpleStubFormEngine. */
@Component({
  selector: 'app-avro-destination-form',
  standalone: true,
  imports: [ReactiveFormsModule],
  templateUrl: './simple-stub-destination-form.component.html',
})
export class AvroDestinationFormComponent implements WizardDestinationFormApi {
  private readonly engine = new SimpleStubFormEngine(inject(FormBuilder), 'Avro Destination', [
    { key: 'filePattern', label: 'File name pattern', placeholder: '{resource}_{yyyyMMdd_HHmmss}.avro' },
  ]);
