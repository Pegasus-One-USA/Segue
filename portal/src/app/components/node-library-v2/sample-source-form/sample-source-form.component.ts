...
/**
 * Thin, independently-editable wrapper around the shared EhrVendorSourceFormComponent engine, fixed to the Sample
 * vendor. Forwards every input/output the engine exposes unchanged — the only thing this file owns is "which
 * vendor" (see the `[vendor]="'Sample'"` binding in the template). Registered in source-form.registry.ts under the
 * `sample` key from sources.data.ts.
 */
@Component({
...
export class SampleSourceFormComponent implements SourceConfigFormComponent {
