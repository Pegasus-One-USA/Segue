 ...
 *  `rule` is the full effective rule as currently resolved (Global/DestinationTypeV2/ResourceType/Field tier;
...
export class DestinationWizardComponent implements OnInit {
  ...
  // never touches the workflow-level `saved` output, PipelineStoreV2, or node.fields at all.
  ...
  // (Destination → [Mapping →] Transformation → De-identification): Mapping (openGroupMapping),
  // Transformation (openRulesForResource — the transformation-rules dialog), and De-identification
  // (the destination's de-identification profile). Whichever node the user clicked opens this screen
  // with the matching tab active, but all three stay switchable from any entry point — see
  // `initialConfigTab` below and NodeLibraryDialogComponent's chain handling.
  readonly configTab = signal<ConfigTab>('mapping');
  /** Which tab Step 3 should activate when this wizard opens — set from the canvas node that opened it
   *  (Mapping node → 'mapping', Transformation node → 'transformation', De-identification node →
   *  'deidentification'). Defaults to 'mapping', matching the pre-V2 behavior. */
  readonly initialConfigTab = input<ConfigTab>('mapping');
  /** Which step this wizard should open on. 1 (Configure) for a brand-new destination; 3 (Map fields) when
   *  a Mapping/Transformation/De-identification chain node opened it against an already-configured
   *  destination, so the user isn't walked back through connection + data-group selection to reach the tab
   *  they clicked. */
  readonly initialStep = input<number>(1);
  /** True when this wizard was opened from a V2 chain node (Mapping / Transformation /
   *  De-identification) rather than to configure the destination itself. In that mode Step 3 IS the
   *  screen — the stepper's Back/Next (and the Review step behind them) aren't meaningful, so the footer
   *  offers a single Save that commits and stays put, letting the user go straight on to another
   *  resource. See saveChainNode(). */
  readonly chainNodeEntry = input<boolean>(false);

  setConfigTab(tab: ConfigTab): void {
    this.configTab.set(tab);
  }

  /** The chain step this wizard was opened for, as its own display name — the header and the save
   *  confirmation identify what the USER clicked (Mapping / Transformation / De-identification), not the
   *  destination that merely hosts its configuration. Null for a normal destination add/edit. */
  readonly chainNodeLabel = computed<string | null>(() => {
    if (!this.chainNodeEntry()) return null;
    switch (this.configTab()) {
      case 'transformation':   return 'Transformation';
      case 'deidentification': return 'De-identification';
      default:                 return 'Mapping';
    }
  });

  /** Header title — the chain step's own name when opened from one of those nodes, otherwise the
   *  destination's (e.g. "SQL Server Configuration"). */
   ...
   *  (field-mapping-list.component) is where the three tabs actually live. */
  readonly listTabForConfigTab = computed<'mappings' | 'transformations' | 'deidentification'>(() => {
    const tab = this.configTab();
    return tab === 'transformation' ? 'transformations'
      : tab === 'deidentification' ? 'deidentification'
      : 'mappings';
  });
   ...
   *  DestinationTypeV2 names. */
  private readonly registryKey = computed<DestinationTypeV2>(() => {
  ...
  private static readonly CSV_TYPES: DestinationTypeV2[] = ['Csv', 'Sftp'];
  private static readonly MONGO_TYPES: DestinationTypeV2[] = ['Mongo'];
  private static readonly MEDPLUM_TYPES: DestinationTypeV2[] = ['Medplum'];
  ...
  constructor() {
    ...
    effect(() => {
      const tab = this.initialConfigTab();
      untracked(() => this.configTab.set(tab));
    });

    // Same one-way "starting value" treatment for the opening step — a chain node (Mapping /
    // Transformation / De-identification) opens straight on Step 3 against its already-configured
    // destination; Back/Next still move freely from there.
    effect(() => {
      const s = this.initialStep();
      untracked(() => {
        if (s > 1) {
          this.step.set(s);
  ...
  /** The real backend DestinationTypeV2 for whichever destination family this wizard instance is
   ...
   *  transformation rule with the right DestinationTypeV2 — same resolution the "Rules" button/modal already
   *  used, just also bound straight into the canvas template now instead of only called from this class. */
  resolveDestinationTypeForRules(): DestinationTypeV2 {
   ...
   *  DestinationTypeV2-scoped TransformationRule applies to it and expects a different type than the column
   ...
   *  table (see WorkflowBuildAssemblerServiceV2.resolveReferenceLookup), which is meaningless without a live
    ...
    this.destinationConfigSvc
      ...
      .pipe(
        map((page) => {
          ...
          // registryKey() already resolves the exact DestinationTypeV2 this wizard instance is configuring
