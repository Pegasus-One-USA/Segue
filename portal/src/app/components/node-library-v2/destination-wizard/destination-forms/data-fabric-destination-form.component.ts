...
export class DataFabricDestinationFormComponent implements WizardDestinationFormApi {
  ...
  getFullConfig(): Record<string, string> {
    ...
    return {
      dest_name: v.name ?? '',
      // OneLake Files is the only implemented landing mode; Warehouse COPY INTO and Eventstream are rejected
      // server-side (see FabricDestinationSettings.Parse), so this form never offers them as a choice.
      dest_fabricMode: 'oneLakeFiles',
