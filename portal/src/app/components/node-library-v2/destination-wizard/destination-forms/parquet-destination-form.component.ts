...
/** Minimal stand-in form for the Parquet destination type — see SimpleStubFormEngine. */
@Component({
  selector: 'app-parquet-destination-form',
  standalone: true,
  imports: [ReactiveFormsModule],
  templateUrl: './simple-stub-destination-form.component.html',
})
export class ParquetDestinationFormComponent implements WizardDestinationFormApi {
  private readonly engine = new SimpleStubFormEngine(inject(FormBuilder), 'Parquet Destination', [
    { key: 'filePattern', label: 'File name pattern', placeholder: '{resource}_{yyyyMMdd_HHmmss}.parquet' },
  ]);
