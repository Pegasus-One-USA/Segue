...
// MVP1 resource set — keep in sync with portal/src/app/data/scope-constants-v2.data.ts's FHIR_RESOURCES.
  ...
  implements OnInit, HasUnsavedChanges, SourceConfigFormComponent
{
   ...
   *  `saved` only fires once WizardServiceV2.save() actually confirms success (see save() below). */
   ...
   *  applies: entity mode (WizardServiceV2.entityId), canvas mode editing an already-built workflow's node
   *  (WizardServiceV2.editingFields()['sourceConnectionId'], embedded there by WorkflowEndpoints on the original
   ...
   * `resources` control instead, regardless of audience — WizardServiceV2.openEntity() seeds it from the existing
  ...
  constructor() {
    // WizardServiceV2.ehrType is the one place downstream code (save()'s SourceConnectionRequest.sourceSystemType,
  ...
  ngOnInit(): void {
    ...
    // default to Epic's own values (see WizardServiceV2.open()/openEntity()) regardless of which vendor form is
    ...
    // open (see WizardServiceV2.open()), and Athenahealth/Healow already correct it via their own overrides
  ...
  /**
   * WizardServiceV2.open() only restores the handful of fields it exposes as named signals (audience, environment,
   ...
   * (see WizardServiceV2.entityDto). CDS Hooks and Full Refresh recurrence have no equivalent below because the
   * backend SourceConnection genuinely doesn't persist them (same caveat WorkflowBuildAssemblerServiceV2's own doc
   ...
   * Mirrors WizardServiceV2.openEntity()'s field mapping, but patches the reactive form directly: this is canvas
  ...
  private runBackendAuthScopeProbe(tokenEndpoint: string): void {
    ...
    if (!clientId || !tokenEndpoint || (method !== 'secret' && method !== 'jwt')) {
      console.info(`[Backend Auth Probe] skipped — clientId=${!!clientId} tokenEndpoint=${!!tokenEndpoint} method=${method}`);
      return;
    }
    if (method === 'secret' && !this.form.controls.clientSecret.value) {
      console.info('[Backend Auth Probe] skipped — Client Secret auth selected but the field is blank (isEditing=' + this.isEditing + '); this connection\'s stored secret will NOT be re-validated by Discover');
      return;
    }
    if (
      method === 'jwt' &&
      ...
      console.info('[Backend Auth Probe] skipped — JWT auth selected but Key Vault Name/Secret Name reference is incomplete');
      return;
    }
    console.info(
    ...
    this.runRealCredentialTest(method, tokenEndpoint).subscribe({
      next: (result) => {
        console.info(`[Backend Auth Probe] result success=${result.success}${result.success ? ` — granted scopes: ${result.grantedScopes.join(', ')}` : ` — ${result.error ?? 'no error message'}`}`);
        if (result.success) {
  ...
  private buildFieldsToSave(
    ...
    return {
      ...
      // WorkflowBuildAssemblerServiceV2.buildSource() still resolves every non-GenericFhir/non-Sample connector to
      ...
      // WizardServiceV2.save()'s own ScopeBuilderService-derived default (which knows nothing about the selected
   ...
   * side-effecting resolution against `existingConnections()`/WizardServiceV2 and doesn't apply to a generic
   * "read the form back out" caller). This form's own footer Save button still goes through save() below, which
   * drives the real WizardServiceV2-backed create/update flow end to end — getFields() exists so this component can
  ...
  protected save(): void {
    ...
    const fieldsToSave: Record<string, string> = {
      ...
      // WorkflowBuildAssemblerServiceV2.assemble() to skip this node's Sources spec entirely and let the backend
