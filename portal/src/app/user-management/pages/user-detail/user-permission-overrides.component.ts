...
export class UserPermissionOverridesComponent implements OnChanges {
  ...
  stateFor(permissionId: string): TriState {
    if (this.inheritFromRole()) {
      return this.rolePermissions.some(p => p.id === permissionId) ? 'grant' : 'inherit';
    }
  ...
  setCellGranted(permissionId: string, granted: boolean): void {
    ...
    if (granted) map.set(permissionId, true); else map.delete(permissionId);
  ...
  setRowState(row: GridRow, next: TriState): void {
    ...
    for (const id of row.permissionIds) {
      if (next === 'inherit') map.delete(id); else map.set(id, next === 'grant');
    }
    this.overrides.set(map);
  }
  ...
  private readonly allPermissionIds = computed<string[]>(() =>
    this.gridCategories().flatMap(cat => cat.rows.flatMap(row => row.permissionIds))
  );
  ...
  save(): void {
    ...
    for (const [id, value] of this.overrides()) permissionIdToIsEnabled[id] = value;

    this.userService.setUserPermissionAllocations(this.userId, permissionIdToIsEnabled).subscribe({
      next: () => {
      ...
      error: () => {
        this.saving.set(false);
        this.errorMessage.set('Failed to save permission overrides. Please try again.');
      },
    });
  }
  ...
  private clearAllOverrides(): void {
    ...
    this.userService.setUserPermissionAllocations(this.userId, {}).subscribe({
      ...
      error: () => {
        this.clearing.set(false);
        this.errorMessage.set('Failed to remove direct permission overrides. Please try again.');
      },
    });
  }
}
