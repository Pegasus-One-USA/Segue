...
import { EPIC_INGESTION } from '../data/ingestion-modes-v2.data';
...
import { SourceNode } from '../models/node-v2.model';
import { AUDIENCE_FIELD_CONFIG, EpicAudience } from '../components/epic-source-wizard-v2/models/audience-field-config.data';
 ...
 *  through EhrVendorSourceFormComponent, which already injects WizardServiceV2. */
...
export class WizardServiceV2 {
  ...
  /** 'canvas' (default): open()/save() read/write a Pipeline Builder canvas node via PipelineStoreV2, exactly as
  ...
  save(formValues: WizardFormValues, step3ModeValues: Record<string, string>): void {
    ...
    // step3ModeValues), not from `this.clientId()` / `this.authMethod()` — those WizardServiceV2 signals are only
    ...
    // The real backend SourceConnection is created later, at workflow build time (WorkflowBuildAssemblerServiceV2),
