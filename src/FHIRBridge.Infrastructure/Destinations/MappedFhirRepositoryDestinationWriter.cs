...
public sealed class MappedFhirRepositoryDestinationWriter : IConfiguredDestinationWriter
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        records = OrderRecordsByFhirReferenceDependency(records);
    ...
    /// <summary>
    /// Orders <paramref name="records"/> so a resource type referenced by another (via any <c>"reference":
    ...
    /// <c>managingOrganization</c> point at them. Derived from the batch's own JSON rather than a static FHIR-
    /// domain table: it only creates an ordering edge when the referenced type is actually present in this same
    /// batch — nothing to reorder against when it isn't (e.g. a Patient referencing an Organization that was never
    /// fetched from the source; that's a separate, upstream gap this can't fix). Same Kahn/DFS topological sort
    /// shape as <c>DestinationNodeExecutors.OrderGroupsByReferenceDependency</c> uses for the relational-destination
    /// path (which instead keys off <see cref="MappedReferenceLookup.LookupTable"/> — a concept FHIR passthrough/
    /// customize records never populate) — cycle-safe, falling back to original order for groups in a cycle rather
    /// than looping. Records with no <see cref="MappedDestinationRecord.SourceJson"/> (the non-FHIR fallback flow)
    /// have no extractable references and keep their original relative position. Stable within a resource type.
    /// </summary>
    private static List<MappedDestinationRecord> OrderRecordsByFhirReferenceDependency(
        IReadOnlyCollection<MappedDestinationRecord> records)
    {
        var groups = records.GroupBy(r => r.ResourceType, StringComparer.OrdinalIgnoreCase).ToList();
        if (groups.Count <= 1)
        {
            return records.ToList();
        }

        var groupByType = groups.ToDictionary(g => g.Key, g => g, StringComparer.OrdinalIgnoreCase);

        var dependencies = groups.ToDictionary(
            g => g,
            g => g
                .SelectMany(r => ExtractReferencedResourceTypes(r.SourceJson))
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .Where(type => groupByType.ContainsKey(type) && !string.Equals(type, g.Key, StringComparison.OrdinalIgnoreCase))
                .Select(type => groupByType[type])
                .ToList());
        ...
        return ordered.SelectMany(g => g).ToList();
    }
    ...
    /// resource-type portion (before the first '/') of each one found. Empty for null/unparseable JSON — the
    /// non-FHIR fallback flow has no references to extract.
    /// </summary>
    private static List<string> ExtractReferencedResourceTypes(string? sourceJson)
    {
        ...
        try
        {
            ...
            WalkForReferences(document.RootElement, referencedTypes);
        }
        catch (JsonException)
        {
    ...
    private static void WalkForReferences(JsonElement element, List<string> referencedTypes)
    {
        switch (element.ValueKind)
        {
            case JsonValueKind.Object:
                foreach (var property in element.EnumerateObject())
                {
                        ...
                        && property.Value.ValueKind == JsonValueKind.String)
                    {
                        ...
                        var slashIndex = reference?.IndexOf('/', StringComparison.Ordinal) ?? -1;
                        if (slashIndex > 0)
                        {
                            referencedTypes.Add(reference![..slashIndex]);
                        }
                    }
                    else
                    {
                        WalkForReferences(property.Value, referencedTypes);
                    }
                }
                break;
            case JsonValueKind.Array:
                ...
                {
                    WalkForReferences(item, referencedTypes);
                }
                break;
        }
    }
