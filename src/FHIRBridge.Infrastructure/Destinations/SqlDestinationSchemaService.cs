...
public sealed class SqlDestinationSchemaService : IDestinationSchemaService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var constraints = await ReadConstraintsAsync(connection, isSqlServerFamily, cancellationToken);
        var autoGenerated = await ReadAutoGeneratedAsync(connection, type, cancellationToken);

        // FK constraints (References) are only readable from SQL Server's own sys.* catalog views —
        // PostgreSQL/MySQL columns keep IsForeignKey=false/References=null (see DestinationColumnSchemaDto's
        // own doc comment).
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            ...
            var (isPrimaryKey, isUnique) = constraints.TryGetValue(columnKey, out var flags)
                ? flags
                : (false, false);
            var references = keyMetadata.TryGetValue(columnKey, out var key) ? key.References : null;
