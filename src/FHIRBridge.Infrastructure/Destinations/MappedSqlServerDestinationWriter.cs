...
/// already exist, and only the mapped destination columns (plus whichever system/audit columns the table actually
/// declares) are ever written. Supports Insert, Upsert (MERGE on the mapped field flagged
/// <see cref="MappingField.IsUpsertKey"/>), Update (updates the matching row by that same key only, never inserts),
/// and CDC write modes. Also resolves FHIR-reference fields against already-written parent tables and writes any
/// child tables (see <see cref="MappingField.ReferenceLookupTable"/> / <see cref="ArrayPolicy.SeparateDestination"/>).
...
public sealed class MappedSqlServerDestinationWriter : IConfiguredDestinationWriter
{
    ...
    // one of these is ignored (the system value wins) so the generated INSERT/MERGE statement never declares a
        ...
        CancellationToken cancellationToken)
    {
        ...
        var columns = systemColumns.Concat(fieldNames).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
        ...
        CancellationToken cancellationToken)
    {
        ...
        // A mapping profile with only the upsert-key field configured (no other columns mapped, and no system
        // columns present on a customer-owned table) leaves updateColumns empty — "UPDATE SET" with nothing after
        // it is invalid T-SQL. There is nothing meaningful to update in that case anyway, so omit the WHEN
        // MATCHED clause entirely.
