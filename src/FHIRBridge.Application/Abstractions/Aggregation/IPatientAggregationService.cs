...
/// patient-compartment resource types from the configured source, fans the queries out in parallel, fully paginates
...
public interface IPatientAggregationService
{
    ...
    /// <paramref name="resourceTypes"/> (<c>{Type}?patient={patientId}</c>) from the configured source connection,
    ...
    /// Optional explicit source connection to read from. When null, the single enabled source is used;
    /// if there are zero or more than one enabled source, resolution fails with a typed error.
    /// </param>
