...
/// <summary>
/// Encrypts and decrypts the identifiers carried in the interactive-OAuth URLs so raw tenant/route GUIDs are never
...
{
    /// <summary>Encrypts (tenantId, routeId) into an opaque, URL-safe token for the registered launch URL.</summary>
    string ProtectContext(Guid tenantId, Guid routeId);
...
/// <summary>The identifiers a launch URL resolves to: the tenant and the pipeline (route) to run.</summary>
public sealed record LaunchContext(Guid TenantId, Guid RouteId);
