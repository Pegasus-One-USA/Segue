...
/// <summary>
/// Evaluates scheduled-pull routes across all tenants and claims the ones that are due.
/// </summary>
public interface IScheduleEvaluationService
{
    /// <summary>
    /// Finds every tenant route whose schedule is due as of <paramref name="utcNow"/> (catch-up aware), stamps each
    /// claimed route's <c>LastTriggeredOnUtc</c> so it is not claimed again, persists the change, and returns the
    /// due runs grouped by tenant. This mutates state — it claims work, it does not merely query it.
    /// </summary>
