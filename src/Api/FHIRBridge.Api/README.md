...
- **UnifiedAdminAuthorizationHandler** / **UnifiedAdminRequirement** — satisfies the `UnifiedAdmin` policy when the principal carries an accepted global role claim, or (for tenant-scoped routes) when `IUserAccessRepository.HasTenantRoleAsync` confirms a GlobalAdmin/TenantAdmin role for the route's `tenantId`.
