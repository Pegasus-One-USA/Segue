...
public abstract class DestinationNodeExecutor : WorkflowNodeExecutorBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (explicitProfile is null && MultiTableRelationalDestinationTypes.Contains(_destinationType))
        {
            ...
            // (Patient -> dbo.Patient_New, Condition -> dbo.Condition, ...) with its own columns/upsert key, natural
            // key resolved against the real MappingProfile, and reference-dependency ordering (a table another
            // group references, e.g. Patient for Observation.PatientId, is written first). Writing the whole mixed
            // batch through one MappingProfile forced every resource type through whichever one's shape happened to
            // be saved first on this node, routing every other resource type at the wrong table/columns and failing
            // with "Invalid column name". Write each resource type's records against its own resolved profile
            // instead.
            //
            ...
            // A REAL MappingProfile's DestinationObject is just the plain table name ("Patient") — splicing the mode
            // back on here is what keeps Upsert/Update working now that every resource-type group resolves its own
            // real profile instead of a single legacy synthetic one with the suffix already baked in.
