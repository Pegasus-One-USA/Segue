...
> The configuration/control-plane domain model — tenants, their connections, mappings, routes, RBAC, and the FHIR vocabulary the platform understands. Pure business rules, no infrastructure.

**Layer:** Domain · **SDK:** Microsoft.NET.Sdk · **Target:** net9.0

## Purpose
`FHIRBridge.Domain` is the innermost layer of the Clean-Architecture solution. It holds the configuration-side domain model that describes *what* a tenant has wired up (sources, destinations, mappings, pipeline routes, webhooks) and *who* may touch it (users, roles, permissions). All entities are persistence-ignorant POCOs with encapsulated state (private setters, factory/`Update` methods) and enforce their own invariants. It depends on nothing but the shared kernel, and every other layer depends on it.

## Responsibilities
- Own the **Tenant aggregate** and its consistency boundary: source connections, webhook configurations, destination configurations, mapping profiles, and resource pipeline routes all hang off `Tenant`, and the aggregate enforces cross-entity rules (membership checks, enable/disable cascades, duplicate-route prevention).
...
- Own the **RBAC model** (users, roles, permissions, and their join entities) including local-login/security hardening state (lockout, password reset, MFA flag) and tenant membership.
...
### Aggregates/
- **`Tenant`** — the aggregate root (`AuditableEntity<Guid>`). Holds private collections of `SourceConnection`, `WebhookConfiguration`, `DestinationConfiguration`, `MappingProfile`, and `ResourcePipelineRoute`, each exposed read-only. Provides factory/update methods (`AddSourceConnection`, `AddMappingProfile`, `AddRoute`, etc.) that validate ownership before mutating, and `Resolve{ResourceType,SourceConnectionId,DestinationId}(route)` helpers that derive a route's type/source/destination from its mapping. Enable/disable methods cascade (disabling a source disables its webhooks → mappings → routes; changing a mapping's source/destination binding disables dependent routes). Also carries governance settings: `RetentionDays`, `TimeZone`, `ContactEmail`, `Region`, and a hard `IsEnabled` kill switch.
...
- **`SourceCapabilityProfile`** — a point-in-time snapshot of a source's CapabilityStatement (`/metadata`): supported `CapabilityResource`s, configured scopes, raw JSON, discovery timestamp. Deliberately a standalone entity (not in the Tenant aggregate) so discovery refreshes independently. `SupportsResourceType` gates whether a mapping may bind to a type.
...
- **`UserActivityAuditLog`** (`Entity<Guid>`) — append-only end-user action record sealed into a per-tenant SHA-256 hash chain (`PreviousHash` + `EntryHash` via `SealChain`) for HIPAA §164.312(b)/(c) tamper evidence.
- **RBAC:** **`User`** (external id, local-login hash, lockout/MFA/password-reset state, home tenant), **`Role`** (system vs. custom, tenant-scoped), **`Permission`** (categorized, system flag), and join entities **`RolePermission`**, **`UserRole`** (global assignment), **`TenantUser`** (per-tenant membership + role). Each join carries an `IsEnabled` soft-disable flag.
...
The skeleton contains only the `FHIRBridge.Domain.csproj` (referencing `FHIRBridge.SharedKernel`) and compiles to an empty assembly. **None** of the entities, aggregates, value objects, enums, or the `Fhir/` helpers listed above have been ported yet. To reach parity with the reference implementation, port (in dependency order): `Enums/` and `ValueObjects/` first, then `Fhir/`, then `Entities/`, then the `Tenant` aggregate, and finally the RBAC and audit entities.
...
- Grow the **Tenant aggregate** with richer governance invariants (data-residency enforcement, retention-window validation, schedule/time-zone rules) while keeping all mutation behind intention-revealing methods.
